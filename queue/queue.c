...
void init() {
    int64storageStore(orderNumberKey, 32, 0);
}

void addMe() {
    i64 orderNumber = int64storageLoad(orderNumberKey, 32);
    setParticipantKeyByIndex(orderNumber);
    getCaller(caller);
    storageStore(participantKey, 32, caller, 32);
    
    int64storageStore(orderNumberKey, 32, ++orderNumber);
    finish("ok", 2);
}
...
void getAt() {
    ...
    storageLoad(participantKey, 32, participant);
    finish(participant, 32);
}

void getCount() {
    i64 counter = int64storageLoad(orderNumberKey, 32);
    int64finish(counter);
}
